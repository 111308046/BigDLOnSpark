...
convUtilsTemplateFile=~/MTP/ForkedRepoBigDL/BigDL/spark/dl/src/main/scala/com/intel/analytics/bigdl/models/mymodel/ConvUtils.template
convTrainTemplateFile=~/MTP/ForkedRepoBigDL/BigDL/spark/dl/src/main/scala/com/intel/analytics/bigdl/models/mymodel/ConvTrain.template
...
while IFS=","
	read config epoch datasize batchsize input_width input_height conv_in conv_out filtersize padding strides output_shape max_core executor_cores task_cpus driver_core executor_instances driver_memory executor_memory parallelism
...
do 
	...
	cp $convTemplateFile $modelFile
	cp $convUtilsTemplateFile $utilsFile
	cp $convTrainTemplateFile $trainFile

	echo step 2/10 !!!!!!!!Filling up the parameter in the model!!!!!!!!!!
	sed -i "s/%%input_width%%/$input_width/g" $modelFile
	sed -i "s/%%input_height%%/$input_height/g" $modelFile
	sed -i "s/%%conv_in%%/$conv_in/g" $modelFile
	sed -i "s/%%conv_out%%/$conv_out/g" $modelFile
	sed -i "s/%%output_shape%%/$output_shape/g" $modelFile
	sed -i "s/%%filtersize%%/$filtersize/g" $modelFile
	...
	sed -i "s/%%conv_in%%/$conv_in/g" $utilsFile
	...
	sed -i "s/%%conv_in%%/$conv_in/g" $trainFile
	...
	dataLoadingTime=$(sshpass -e ssh -n spark@10.129.2.177 "kubectl logs convcustommodel$config |  sed -n '/Time took to load the data/{n;p;}' ")
	valDataLoadTime=$(sshpass -e ssh -n spark@10.129.2.177 "kubectl logs convcustommodel$config |  sed -n '/Time took to load the validation data/{n;p;}' ")
	dataCacheTime=$(sshpass -e ssh -n spark@10.129.2.177 "kubectl logs convcustommodel$config |  sed -n '/dataload Cache time/{n;p;}' " | tr "\n" ",")
	result=$(sshpass -e ssh -n spark@10.129.2.177 "kubectl logs convcustommodel$config | grep InstrumentationResult")
	
	sshpass -e ssh -n spark@10.129.2.177 "kubectl logs convcustommodel$config" > /home/mayur/MTP/automateBigDL/MTP2/logs/$config
	

	echo step 10/10 !!!!!!!Storing the result!!!!!!!!
	
	cd - && echo "$config,$epoch,$datasize,$batchsize,$input_width,$input_height,$conv_in,$conv_out,$filtersize,$padding,$strides,$output_shape,$max_core,$executor_cores,$task_cpus,$driver_core,$executor_instances,$driver_memory,$executor_memory,$parallelism,$dataLoadingTime,$valDataLoadTime,$result" >>convoutput.csv
	echo -------------------------------------------------
done < <(tail -n +3 /home/mayur/MTP/automateBigDL/MTP2/jobrun/convinputdata.csv)
cat convoutput.csv >> convpers.csv
